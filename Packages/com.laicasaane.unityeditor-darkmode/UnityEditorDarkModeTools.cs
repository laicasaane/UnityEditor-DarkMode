...
namespace UnityEditorDarkMode.Tools
{
    ...
    {
        private const string PACKAGE_NAME = "com.laicasaane.unityeditor-darkmode";
        private const string DISABLED_PLUGINS_FOLDER = "Plugins~";
        private const string ENABLED_PLUGINS_FOLDER = PACKAGE_NAME;
        ...
        private static bool ValidateEnableDarkMode()
        {
            ...
            if (string.IsNullOrEmpty(projectPluginsPath) || string.IsNullOrEmpty(packageRootPath))
            {
                return false;
            }
            ...
            return Directory.Exists(disabledFolder) && Directory.Exists(enabledFolder) == false;
        }
        ...
        private static void TogglePluginsFolder(string packageRootPath, string projectPluginsPath, bool enabled)
        {
            ...
            else if (enabled == false && Directory.Exists(enabledFolderPath))
            {
                var result = EditorUtility.DisplayDialogComplex(
                    ...
                    , "Show in Explorer and Exit Editor"
                    , "Cancel"
                    , "Show in Explorer"
                );
                ...
                if (result == 0)
                {
                    EditorUtility.RevealInFinder(enabledFolderPath);
